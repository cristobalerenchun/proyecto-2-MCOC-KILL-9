...
a = Z
println("satelite listo")
xdoc_in = parse_file("orbita-euler-mejorado.xml")
...
for hijo in child_nodes(List_of_OSVs)
	...
	if is_elementnode(hijo)
		...
		if nombre == "OSV"
			...
			#println("i = $(i) utc_texto = $(utc_texto) now = $(now)")
			t[i] = UnixTime
			ZZ[1,i] = parse(Float64, content(XMLElement(hijo)["X"][1]))
			ZZ[2,i] = parse(Float64, content(XMLElement(hijo)["Y"][1]))
			ZZ[3,i] = parse(Float64, content(XMLElement(hijo)["Z"][1]))
			ZZ[4,i] = parse(Float64, content(XMLElement(hijo)["VX"][1]))
			ZZ[5,i] = parse(Float64, content(XMLElement(hijo)["VY"][1]))
			ZZ[6,i] = parse(Float64, content(XMLElement(hijo)["VZ"][1]))
			i += 1
		end	
	end
end
b = ZZ
println("orbita listo")
println("SATELITE VS EULER MEJORADO")

difft = maximum(a[1:9000] - b[1:9000])
diffX = a[1:3,1:9000] - b[1:3,1:9000]
